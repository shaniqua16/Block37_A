...
const postReview = async (req, res, next) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .send({ message: "Rating must be an number  between 1 and 5." });
    }
    ...
    if (!itemExists) {
      return res
        .status(404)
        .send({ message: "Cannot review an item that does not exist." });
    }
    ...
    if (
      ...
      error.meta?.target?.includes("itemId")
    ) {
      return res
        .status(409)
        .send({ message: "You have already reviewed this item." }); // 409 Conflict
    }
    next(error);
  }
};
...
const getReviewById = async (req, res, next) => {
  try {
    ...
    if (!review || review.itemId !== itemId) {
      return res
        .status(404)
        .send({ message: "Review not found fot this item" });
    }
    res.send(review);
  } catch (error) {
    next(error);
